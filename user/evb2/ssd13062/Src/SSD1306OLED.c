...
uint8_t i2caddr, vccstate, x_pos, y_pos, text_size;
bool wrap = TRUE;
...
void SSD1306_I2C_init(void){
	I2C_init(SSD1306_I2C_ADDRESS, 2, I2C_PC67);							// initialize I2C port, addr 0x3C @420KHz
}
...
void SSD1306_OLED_init(void){
	...
	ssd1306_command(0x22);																	// precharge value 0x22
	ssd1306_command(SSD1306_SETCONTRAST);                   // 0x81
	ssd1306_command(0x8F);
	ssd1306_command(SSD1306_SEGREMAP | 0x1);								// rotate 180
	ssd1306_command(SSD1306_COMSCANDEC);										// rotate 180
...
void SSD1306_DisplayRotate(uint8_t r){
	if(r == 0){
		...
		ssd1306_command(SSD1306_SEGREMAP_INV);
		ssd1306_command(SSD1306_COMSCANDEC);
	}
}
...
void SSD1306_StartScrollRight(uint8_t start, uint8_t stop)
{
  ...
  ssd1306_command(start & 0x7);
  ssd1306_command(0X00 & 0x7 );
  ssd1306_command(stop & 0x7);
  ssd1306_command(0X00);
...
void SSD1306_LeftHScrol(uint8_t start_page, uint8_t frame, uint8_t end_page){
	ssd1306_command(SSD1306_LEFT_HORIZONTAL_SCROLL);
	ssd1306_command(0x00);
	ssd1306_command(start_page & 0x7);
	ssd1306_command(frame & 0x7);
	ssd1306_command(end_page & 0x7);
	ssd1306_command(0x00);
	ssd1306_command(0xFF);
	ssd1306_command(SSD1306_ACTIVATE_SCROLL);
}
...
void SSD1306_DisplayOnOFF(bool onOff){
	if(onOff) ssd1306_command(SSD1306_DISPLAYON);
	else ssd1306_command(SSD1306_DISPLAYOFF);
}
...
void SSD1306_ROMBMP(uint8_t x, uint8_t y, uint8_t *bitmap, uint8_t w, uint8_t h)
{
  ...
  {    
    ...
    {      
      if( (bitmap[j/8 + i*w], j % 8) == 1 )			// need bit test method
